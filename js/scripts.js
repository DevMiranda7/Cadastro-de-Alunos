...
function CadastrarAluno(nome,idade,nota,alunos) {
  let aluno = new Aluno(nome,idade,nota);
  if(!alunos.some(x => x.Nome == nome)){
    alunos.push(aluno);
    return aluno;
  }
...
function OrdenarPorNota(alunos) {
 let notasOrganizadas = [];
 notasOrganizadas.push(alunos);
 notasOrganizadas.sort((a,b) => a.Nota - b.Nota)
...
let idadeOrganizada = [];
idadeOrganizada.push(alunos)
return idadeOrganizada.sort((a,b) => a.Idade - b.Idade);
...
const mediaCalculada = soma / alunos.length;
